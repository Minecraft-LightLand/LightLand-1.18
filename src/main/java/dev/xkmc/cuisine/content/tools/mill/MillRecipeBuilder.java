...
public class MillRecipeBuilder extends BaseRecipeBuilder<MillRecipeBuilder, MillRecipe, MillRecipe, MillBlockEntity.RecipeContainer> {
	...
	public MillRecipeBuilder(ItemLike result, int count, int time, FluidStack in, FluidStack out) {
	...
	public MillRecipeBuilder requires(TagKey<Item> tagKey) {
		return this.requires(Ingredient.of(tagKey));
	}

	public MillRecipeBuilder requires(ItemLike itemLike) {
		return this.requires(itemLike, 1);
	}

	public MillRecipeBuilder requires(ItemLike itemLike, int count) {
	...
	public MillRecipeBuilder requires(Ingredient ingredient) {
		return this.requires(ingredient, 1);
	}

	public MillRecipeBuilder requires(Ingredient ingredient, int count) {
		for (int i = 0; i < count; ++i) {
