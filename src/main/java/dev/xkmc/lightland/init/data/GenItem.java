...
import dev.xkmc.lightland.content.common.item.generic.*;
...
public class GenItem {
	...
	public static ItemEntry<Item>[][] genItem() {
		...
		for (int i = 0; i < n; i++) {
			...
			BiFunction<String, EquipmentSlot, ItemEntry> armor_gen = (str, slot) ->
					mat.armor_config.func.apply(LightLand.REGISTRATE.item(id + str,
			...
			BiFunction<String, Tools, ItemEntry> tool_gen = (str, tool) ->
					mat.tool_config.func.apply(LightLand.REGISTRATE.item(id + str,
	...
	public static ItemEntry<Item>[] genMats(String suffix) {
		...
		for (int i = 0; i < n; i++) {
			ans[i] = LightLand.REGISTRATE.item(Mats.values()[i].id + suffix, Item::new).defaultModel().defaultLang().register();
		}
		return ans;
	}
	...
	public static BlockEntry<Block>[] genBlockMats() {
		...
		for (int i = 0; i < n; i++) {
			ans[i] = LightLand.REGISTRATE.block(Mats.values()[i].id + "_block", p -> new Block(Block.Properties.copy(Blocks.IRON_BLOCK)))
