...
public class FireSword extends Arcane {
	...
	public boolean activate(Player player, LLPlayerData magic, ItemStack stack, LivingEntity target) {
		...
		if (!w.isClientSide()) {
			...
			EffectAddUtil.forceAddEffect(target, new MobEffectInstance(VanillaMagicRegistrate.FLAME.get(), time, 1),
					VanillaMagicRegistrate.FLAME.get(), player);
		}
		return true;
	}
	...
	private void strike(Level w, Player player, LivingEntity target) {
		if (!w.isClientSide()) {
			EffectAddUtil.forceAddEffect(target, new MobEffectInstance(VanillaMagicRegistrate.FLAME.get(), time, 0),
					VanillaMagicRegistrate.FLAME.get(), player);
		}
	}
